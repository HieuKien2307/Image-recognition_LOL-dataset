...
def resnet34(pretrained=True, progress=True, out_classes = 64, **kwargs):
...
def resnet50(pretrained=True, progress=True, out_classes = 64, **kwargs):
...
def resnet101(pretrained = True, progress=True,out_classes = 64, **kwargs):
    model = _resnet(
        "resnet101", Bottleneck, [3, 4, 23, 3], pretrained, progress, num_classes = 1000, **kwargs
    )
    model.fc = nn.Linear(2048, out_classes)
    return model


def resnet50_v2(pretrained=True, progress=True,out_classes = 64, **kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
